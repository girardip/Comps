...
```{r Removing Individuals with Too High Wages}
nrow(complete.df)
complete.df = complete.df %>% filter(realWage < 1e3)
nrow(complete.df)
```
...
```{r}
set.seed(1)
sampleInd = sample(complete.df$idind, size = length(complete.df$idind)/400)
sample_complete.df =  complete.df %>% filter(idind %in% sampleInd)
```


```{r Fitting fixef}
fixed = plm(realWage ~ log(PriceIndex) + RealGDP + 
                (State +Gender + Ethnicity + Literacy +
                   Schooling + EmploymentType +
                   Age + I(Age^2) + as.factor(Month)+
                   veryVulnerable)*log(PriceIndex) -State -Gender -Ethnicity -veryVulnerable -1 + timeDiscrete2,
            data = complete.df %>% mutate(timeDiscrete2=as.integer(timeDiscrete)), index = c("idind", "timeDiscrete"), model = "fd") 
# running with reduced sample
fixed2 = plm(log(realWage) ~ log(PriceIndex) + RealGDP + 
                (State +Gender + Ethnicity + Literacy +
                   Schooling + EmploymentType +
                   Age + I(Age^2) + as.factor(Month)+
                   veryVulnerable)*log(PriceIndex) -State -Gender -Ethnicity -veryVulnerable -1 +
               timeDiscrete2 + as.factor(Month)*State,
            data = complete.df %>% mutate(timeDiscrete2=as.integer(timeDiscrete)), index = c("idind", "timeDiscrete"), model = "fd") 
# running with reduced sample
fixed2_MonthsReduced = plm(log(realWage) ~ log(PriceIndex) + RealGDP + 
                (State +Gender + Ethnicity + Literacy +
                   Schooling + EmploymentType +
                   Age + I(Age^2) + MonthJan+
                   veryVulnerable)*log(PriceIndex) -State -Gender -Ethnicity -veryVulnerable -1 +
               timeDiscrete2 + MonthJan*State,
            data = sample_complete.df %>% mutate(timeDiscrete2=as.integer(timeDiscrete),
                                                 MonthJan=as.factor(ifelse(Month==1,T,F))), index = c("idind", "timeDiscrete"), model = "fd") 
# running with reduced sample
```

maybe useful? https://cran.r-project.org/web/packages/wfe/wfe.pdf

```{r}
summary(fixed)
summary(fixed2)
summary(fixed2_MonthsReduced)
```

## Variable Selection

```{r}
fixed_vcovHC1 = coeftest(fixed2, vcov. = vcovHC)
fixed_vcovHC1
```
