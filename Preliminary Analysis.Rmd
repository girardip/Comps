...
````{r Importing the Actual Full Dataset, eval=T}
# Importing datset and selecting relevant variables
fullDf = data.table::fread("~/Comps/PME/PME_csv/reduced_df8.csv")
fullDf= fullDf %>% 
  select(v035, v040, v050, v070, v203, v4191, v208, v301, v307, v234)

# Fixing Variables v301 and v307 ()
# v301 gives NA for all kids. no big deal as all Nas there are Nas in income anyway
# make v307 NA into an "Other" category
fullDf$v307 = ifelse(is.na(fullDf$v307), "Other", fullDf$v307)

````


```{r Relabeling Factors, eval=T}
...
dictionaryPME = dictionaryPME %>%
  group_by(cod) %>%
  mutate(inicio = max(inicio, na.rm = T)) #fixing NAs again
...
originalColNames = str_to_upper(colnames(fullDf))
...
#   }
# }
# finalColNames = str_replace_all(originalColNames, " ", "")
# colnames(fullDf) = finalColNames
```
