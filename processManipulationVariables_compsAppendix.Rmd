...
reducedPME = reducedPME %>%  
  group_by(v035, month_year = as.factor(paste0(as.character(v070),as.character(v075)))) %>% 
...
```{r Removing Poor People and People without 8 obs}
reducedPME = reducedPME %>% 
  filter(nObsPoor > 0, 
         nObsInd == 8)

# ggplot(onlypoorwages, aes(x=as.factor(n), y=meanWage)) + geom_boxplot(outlier.shape = NA) + ylim(c(0,500)) + theme_minimal() + labs(y="Individual Mean Real Wage", x="Number of individual observations")
```
...
```{r}
fwrite(reducedPME, "complete2.csv")
```
