...
length(stateIndex)*length(educIndex)*length(monthIndex)*length(vulnerableIndex)
...
for (i in 1:nrow(cases.df[2,])){
  filteredVcov= vcovInteractions
  indexRed = stateIndex[-cases.df$State[i]]
  filteredVcov = ifelse(length(indexRed)!=0,
                         filteredVcov[-indexRed, -indexRed],
                         filteredVcov[-stateIndex[-1],-stateIndex[-1]])
  print(paste0(i, rownames(filteredVcov), filteredVcov))
}
