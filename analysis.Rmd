...
originalNrow = nrow(vcovInteractions)
for (i in 1:nrow(cases.df[1,])){
  filteredVcov = vcovInteractions
  indexRed = stateIndex[-cases.df$State[i]]
  if (length(indexRed)!=0) {
    filteredVcov = filteredVcov[-indexRed[-1], -indexRed[-1]]
  } else {
    filteredVcov = filteredVcov[-stateIndex[-1],-stateIndex[-1]]
  }
  print(paste(i, sum(filteredVcov)))
  modNrow = nrow(filteredVcov)
    for (i in 1:nrow(cases.df[1,])){
    indexRed = educIndex[-cases.df$Schooling[i]]
    if (length(indexRed)!=0) {
      filteredVcov = filteredVcov[-indexRed[-1], -indexRed[-1]]
    } else {
    ...
    print(paste(i, sum(filteredVcov)))
      ...
      for (i in 1:nrow(cases.df[1,])){
      indexRed = monthIndex[-cases.df$Month[i]]
      if (length(indexRed)!=0) {
        filteredVcov = filteredVcov[-indexRed[-1], -indexRed[-1]]
      } else {
      ...
      print(paste(i, sum(filteredVcov)))
          ...
          for (i in 1:nrow(cases.df[1,])){
          indexRed = vulnerableIndex[-cases.df$veryVulnerable[i]]
          if (length(indexRed)!=0) {
            filteredVcov = filteredVcov[-indexRed[-1], -indexRed[-1]]
          } else {
            filteredVcov = filteredVcov[-vulnerableIndex[-1],-vulnerableIndex[-1]]
          }
          print(paste(i, sum(filteredVcov)))
}}}}

  # inside the loop, add upp all the entries in the matrix to get the Variance

# convert variances into SEs
```
